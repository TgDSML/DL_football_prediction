...
class CNNTrainer:
    ...
    def validate(self, val_loader: DataLoader | None):
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc="Validating hybrid", leave=False):
                sequences = batch["sequences"].to(self.device)
                labels = batch["labels"].to(self.device)
    ...
    def fit(self, train_loader: DataLoader, val_loader: DataLoader | None):
        ...
        print(f"\nTraining hybrid for {self.num_epochs} epochs...")
    ...
    def predict(self, test_loader: DataLoader):
        ...
        with torch.no_grad():
            for batch in tqdm(test_loader, desc="Predicting hybrid", leave=False):
                sequences = batch["sequences"].to(self.device)
                labels = batch["labels"].to(self.device)
...
def build_model_for_hybrid(train_dataset, num_classes: int, dropout: float):
...
def should_drop_window_feature(column_name: str) -> bool:
    return any(
        column_name.startswith(prefix) and column_name.endswith(suffix)
        for prefix in ROLLING_BASE_PREFIXES
        for suffix in DROP_WINDOW_SUFFIXES
    )


def filter_keep_only_window5(df: pd.DataFrame, split_name: str = "dataset") -> pd.DataFrame:
    cols_to_drop = [c for c in df.columns if should_drop_window_feature(c)]
    filtered_df = df.drop(columns=cols_to_drop, errors="ignore")

    print(f"[{split_name}] removed {len(cols_to_drop)} rolling columns from windows 3 and 10")
    print(f"[{split_name}] shape: {df.shape} -> {filtered_df.shape}")
    if cols_to_drop:
        print(f"[{split_name}] first removed columns: {cols_to_drop[:15]}")

    kept_window5 = [c for c in filtered_df.columns if any(c.startswith(prefix) and c.endswith('_5') for prefix in ROLLING_BASE_PREFIXES)]
    print(f"[{split_name}] kept {len(kept_window5)} rolling window-5 columns")
    if kept_window5:
        print(f"[{split_name}] first kept _5 columns: {kept_window5[:15]}")
...
def run_single_experiment(device: torch.device):
    print("\n" + "=" * 80)
    print(f"RUNNING HYBRID CNN | KEEP WINDOW-5 ONLY | k={SEQUENCE_LENGTH} | lr={LEARNING_RATE} | dropout={DROPOUT} | focal")
    print("=" * 80)

    data_module = create_data_module()

    for attr in ["train_dataset", "val_dataset", "test_dataset"]:
        ds = getattr(data_module, attr)
        if ds is None:
            continue
        if hasattr(ds, "df") and isinstance(ds.df, pd.DataFrame):
            ds.df = filter_keep_only_window5(ds.df, split_name=attr)
        if hasattr(ds, "data") and isinstance(ds.data, pd.DataFrame):
            ds.data = filter_keep_only_window5(ds.data, split_name=attr)
    ...
    model = build_model_for_hybrid(train_dataset, num_classes, DROPOUT)
    ...
    results_dir = PROJECT_ROOT / "artifacts" / "cnn_results" / f"hybrid_k{SEQUENCE_LENGTH}_keep5_focal"
    results_dir.mkdir(parents=True, exist_ok=True)

    model_dir = PROJECT_ROOT / "outputs" / "models"
    model_dir.mkdir(parents=True, exist_ok=True)
    model_path = model_dir / f"hybrid_k{SEQUENCE_LENGTH}_keep5_focal.pth"
    ...
    if test_loader is not None:
        test_preds, test_proba, test_labels = trainer.predict(test_loader)
        evaluate_and_save_metrics(
            y_true=test_labels,
            y_pred=test_preds,
            y_proba=test_proba,
            output_dir=results_dir,
            prefix=f"hybrid_k{SEQUENCE_LENGTH}_keep5_focal_test",
        )
...
def main():
    print("=" * 80)
    print("HYBRID CNN KEEP ONLY WINDOW-5 ROLLING FEATURES")
    print("=" * 80)
    ...
    result = run_single_experiment(device)
    comparison_df = pd.DataFrame([result])

    comparison_dir = PROJECT_ROOT / "artifacts" / "cnn_results"
    comparison_dir.mkdir(parents=True, exist_ok=True)
    comparison_path = comparison_dir / "hybrid_k10_keep5_focal.csv"
