...
def create_data_module(batch_size: int):
...
def run_single_experiment(batch_size: int, device: torch.device):
    print("\n" + "=" * 80)
    print(f"RUNNING HYBRID MODEL | k={SEQUENCE_LENGTH} | batch_size={batch_size}")
    print("=" * 80)

    data_module = create_data_module(batch_size)
    ...
    results_dir = PROJECT_ROOT / "artifacts" / "cnn_results" / f"hybrid_k{SEQUENCE_LENGTH}_bs{batch_size}"
    results_dir.mkdir(parents=True, exist_ok=True)

    model_dir = PROJECT_ROOT / "outputs" / "models"
    model_dir.mkdir(parents=True, exist_ok=True)
    model_path = model_dir / f"hybrid_k{SEQUENCE_LENGTH}_bs{batch_size}.pth"
    ...
    if test_loader is not None:
        ...
        evaluate_and_save_metrics(
            ...
            prefix=f"hybrid_k{SEQUENCE_LENGTH}_bs{batch_size}_test",
        )
...
def main():
    print("=" * 80)
    print("HYBRID CNN k=10 BATCH SIZE SWEEP")
    print("=" * 80)
    ...
    for batch_size in BATCH_SIZES:
        comparison_rows.append(run_single_experiment(batch_size, device))
    ...
    comparison_path = comparison_dir / "hybrid_k10_batch_size_sweep.csv"
