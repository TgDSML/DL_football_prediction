...
def drop_rolling_features(df: pd.DataFrame) -> pd.DataFrame:
    cols_to_drop = [c for c in df.columns if any(c.startswith(prefix) for prefix in ROLLING_PREFIXES)]
    return df.drop(columns=cols_to_drop, errors="ignore")


def run_single_experiment(device: torch.device):
    print("\n" + "=" * 80)
    print(f"RUNNING HYBRID MODEL | k={SEQUENCE_LENGTH} | lr={LEARNING_RATE} | dropout={DROPOUT} | focal")
    print("=" * 80)
    ...
    for attr in ["train_dataset", "val_dataset", "test_dataset"]:
        ...
        if hasattr(ds, "df") and isinstance(ds.df, pd.DataFrame):
            ds.df = drop_rolling_features(ds.df)
        if hasattr(ds, "data") and isinstance(ds.data, pd.DataFrame):
            ds.data = drop_rolling_features(ds.data)
    ...
    results_dir = PROJECT_ROOT / "artifacts" / "cnn_results" / f"hybrid_k{SEQUENCE_LENGTH}_no_rolling_focal"
    results_dir.mkdir(parents=True, exist_ok=True)

    model_dir = PROJECT_ROOT / "outputs" / "models"
    model_dir.mkdir(parents=True, exist_ok=True)
    model_path = model_dir / f"hybrid_k{SEQUENCE_LENGTH}_no_rolling_focal.pth"
    ...
    if test_loader is not None:
        ...
        evaluate_and_save_metrics(
            ...
            prefix=f"hybrid_k{SEQUENCE_LENGTH}_no_rolling_focal_test",
        )
...
def main():
    print("=" * 80)
    print("HYBRID CNN k=10 NO-ROLLING ABLACTION")
    print("=" * 80)
    ...
    comparison_path = comparison_dir / "hybrid_k10_no_rolling_focal.csv"
