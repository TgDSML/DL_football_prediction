...
DROPOUT = 0.3
LOSS_CONFIGS = [
    {"use_focal_loss": True, "focal_gamma": 3.5, "label": "focal"},
    {"use_focal_loss": False, "focal_gamma": None, "label": "cross_entropy"},
]
...
def run_single_experiment(loss_config: dict, device: torch.device):
    loss_label = loss_config["label"]
    use_focal_loss = loss_config["use_focal_loss"]
    focal_gamma = loss_config["focal_gamma"]

    print("\n" + "=" * 80)
    print(
        f"RUNNING HYBRID MODEL | k={SEQUENCE_LENGTH} | lr={LEARNING_RATE} | "
        f"dropout={DROPOUT} | loss={loss_label}"
    )
    print("=" * 80)
    ...
    results_dir = PROJECT_ROOT / "artifacts" / "cnn_results" / f"hybrid_k{SEQUENCE_LENGTH}_{loss_label}"
    results_dir.mkdir(parents=True, exist_ok=True)

    model_dir = PROJECT_ROOT / "outputs" / "models"
    model_dir.mkdir(parents=True, exist_ok=True)
    model_path = model_dir / f"hybrid_k{SEQUENCE_LENGTH}_{loss_label}.pth"
    ...
    result_row = {
        ...
        "use_focal_loss": use_focal_loss,
        "focal_gamma": focal_gamma if focal_gamma is not None else np.nan,
    ...
    if test_loader is not None:
        ...
        evaluate_and_save_metrics(
            ...
            prefix=f"hybrid_k{SEQUENCE_LENGTH}_{loss_label}_test",
        )
...
def main():
    print("=" * 80)
    print("HYBRID CNN k=10 LOSS FUNCTION SWEEP")
    print("=" * 80)
    ...
    for loss_config in LOSS_CONFIGS:
        comparison_rows.append(run_single_experiment(loss_config, device))
    ...
    comparison_path = comparison_dir / "hybrid_k10_loss_sweep.csv"
