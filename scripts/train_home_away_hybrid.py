"""Standalone training script for the home/away hybrid CNN model."""
...
from src.models.home_away_hybrid import build_home_away_hybrid_cnn

MODEL_TYPE = "home_away_hybrid"
SEQUENCE_LENGTH = 10
BATCH_SIZE = 32
NUM_EPOCHS = 60
LEARNING_RATE = 5e-4
...
class CNNTrainer:
    ...
    def _prepare_inputs(self, sequences: torch.Tensor, model_type: str) -> torch.Tensor:
        if model_type in {"conv1d", "hybrid", "home_away_hybrid"}:
            return sequences
...
def main():
    print("=" * 70)
    print("HOME/AWAY HYBRID CNN TRAINING")
    print("=" * 70)
    ...
    model = build_home_away_hybrid_cnn(
    ...
    print(f"Expected input: (N, k, F) = (_, {seq_len}, {num_features})")
    ...
    output_dir = PROJECT_ROOT / "artifacts" / "cnn_results" / f"home_away_hybrid_k{SEQUENCE_LENGTH}"
    ...
    if test_loader is not None:
        ...
        evaluate_and_save_metrics(
            ...
            prefix=f"home_away_hybrid_k{SEQUENCE_LENGTH}_test",
        )
    else:
        print("\n[Step 4] No test set available.")

    print("\n[Step 5] Saving model and history...")
    model_dir = PROJECT_ROOT / "outputs" / "models"
    model_dir.mkdir(parents=True, exist_ok=True)
    model_path = model_dir / f"home_away_hybrid_k{SEQUENCE_LENGTH}.pth"
