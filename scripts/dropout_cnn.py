...
def run_single_experiment(dropout: float, device: torch.device):
    print("\n" + "=" * 80)
    print(f"RUNNING HYBRID MODEL | k={SEQUENCE_LENGTH} | dropout={dropout}")
    print("=" * 80)
    ...
    results_dir = PROJECT_ROOT / "artifacts" / "cnn_results" / f"hybrid_k{SEQUENCE_LENGTH}_dropout{dropout}"
    results_dir.mkdir(parents=True, exist_ok=True)

    model_dir = PROJECT_ROOT / "outputs" / "models"
    model_dir.mkdir(parents=True, exist_ok=True)
    model_path = model_dir / f"hybrid_k{SEQUENCE_LENGTH}_dropout{dropout}.pth"
    ...
    if test_loader is not None:
        ...
        evaluate_and_save_metrics(
            ...
            prefix=f"hybrid_k{SEQUENCE_LENGTH}_dropout{dropout}_test",
        )
...
def main():
    print("=" * 80)
    print("HYBRID CNN k=10 DROPOUT SWEEP")
    print("=" * 80)
    ...
    for dropout in DROPOUTS:
        comparison_rows.append(run_single_experiment(dropout, device))
    ...
    comparison_path = comparison_dir / "hybrid_k10_dropout_sweep.csv"
