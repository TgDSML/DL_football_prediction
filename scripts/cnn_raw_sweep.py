...
from src.models.raw_cnn import Conv2DMatchPredictor
...
def to_conv2d_input(x: torch.Tensor) -> torch.Tensor:
    if x.ndim != 3:
        raise ValueError(f"Expected 3D tensor (N, k, F) for conv2d conversion, got {tuple(x.shape)}")
...
def fit_single_run(
    ...
    run_id = (
        f"{stage}_conv2d_seq{SEQUENCE_LENGTH}"
