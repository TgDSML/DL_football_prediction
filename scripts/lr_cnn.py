...
class CNNTrainer:
    ...
    def _prepare_inputs(self, sequences: torch.Tensor) -> torch.Tensor:
        return sequences
    ...
    def train_epoch(self, train_loader: DataLoader):
        ...
        for batch in pbar:
            ...
            logits = self.model(self._prepare_inputs(sequences))
...
def run_single_experiment(learning_rate: float, device: torch.device):
    print("\n" + "=" * 80)
    print(f"RUNNING HYBRID MODEL | k={SEQUENCE_LENGTH} | lr={learning_rate}")
    print("=" * 80)
    ...
    results_dir = PROJECT_ROOT / "artifacts" / "cnn_results" / f"hybrid_k{SEQUENCE_LENGTH}_lr{learning_rate}"
    results_dir.mkdir(parents=True, exist_ok=True)

    model_dir = PROJECT_ROOT / "outputs" / "models"
    model_dir.mkdir(parents=True, exist_ok=True)
    model_path = model_dir / f"hybrid_k{SEQUENCE_LENGTH}_lr{learning_rate}.pth"
    ...
    if test_loader is not None:
        ...
        evaluate_and_save_metrics(
            ...
            prefix=f"hybrid_k{SEQUENCE_LENGTH}_lr{learning_rate}_test",
        )
...
def main():
    print("=" * 80)
    print("HYBRID CNN k=10 LEARNING RATE SWEEP")
    print("=" * 80)
    ...
    for lr in LEARNING_RATES:
        comparison_rows.append(run_single_experiment(lr, device))
    ...
    comparison_path = comparison_dir / "hybrid_k10_lr_sweep.csv"
