...
RUN_LABEL = f"hybrid_k{SEQUENCE_LENGTH}_no_avg_focal"

# only these rolling average columns are dropped — everything else stays
DROP_AVG_PREFIXES = [
    "avg_form_",
    "win_rate_",
    "gf_avg_",
...
class CNNTrainer:
    ...
    def train_epoch(self, train_loader: DataLoader):
        ...
        pbar = tqdm(train_loader, desc=f"Training {RUN_LABEL}", leave=False)
        for batch in pbar:
    ...
    def validate(self, val_loader: DataLoader | None):
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc=f"Validating {RUN_LABEL}", leave=False):
                sequences = batch["sequences"].to(self.device)
                labels = batch["labels"].to(self.device)
    ...
    def fit(self, train_loader: DataLoader, val_loader: DataLoader | None):
        ...
        print(f"\nTraining {RUN_LABEL} for {self.num_epochs} epochs...")
    ...
    def predict(self, test_loader: DataLoader):
        ...
        with torch.no_grad():
            for batch in tqdm(test_loader, desc=f"Predicting {RUN_LABEL}", leave=False):
                sequences = batch["sequences"].to(self.device)
                labels = batch["labels"].to(self.device)
...
def filter_no_avg(df: pd.DataFrame, split_name: str = "dataset") -> pd.DataFrame:
    cols_to_drop = [
        c for c in df.columns
        if any(c.startswith(prefix) for prefix in DROP_AVG_PREFIXES)
    ]
    filtered_df = df.drop(columns=cols_to_drop, errors="ignore")

    print(f"[{split_name}] removed {len(cols_to_drop)} rolling average columns")
    print(f"[{split_name}] shape: {df.shape} -> {filtered_df.shape}")
    if cols_to_drop:
        print(f"[{split_name}] first dropped: {cols_to_drop[:15]}")
...
def run_experiment(device: torch.device):
    print("\n" + "=" * 80)
    print(
        f"HYBRID CNN | NO ROLLING AVERAGES | k={SEQUENCE_LENGTH} | "
        f"lr={LEARNING_RATE} | dropout={DROPOUT} | focal(gamma={FOCAL_GAMMA})"
    )
    print("=" * 80)
    ...
    for attr in ["train_dataset", "val_dataset", "test_dataset"]:
        ...
        if hasattr(ds, "df") and isinstance(ds.df, pd.DataFrame):
            ds.df = filter_no_avg(ds.df, split_name=attr)
        if hasattr(ds, "data") and isinstance(ds.data, pd.DataFrame):
            ds.data = filter_no_avg(ds.data, split_name=attr)
    ...
    print(f"\ntrain_dataset.sequences.shape: {train_dataset.sequences.shape}")
    if val_dataset is not None:
        print(f"val_dataset.sequences.shape:   {val_dataset.sequences.shape}")
    if test_dataset is not None:
        print(f"test_dataset.sequences.shape:  {test_dataset.sequences.shape}")
    ...
    class_weights_numpy[nonzero_mask] = len(train_labels) / (
        num_classes * class_counts[nonzero_mask]
    )
    ...
    results_dir = PROJECT_ROOT / "artifacts" / "cnn_results" / RUN_LABEL
    results_dir.mkdir(parents=True, exist_ok=True)

    model_dir = PROJECT_ROOT / "outputs" / "models"
    model_dir.mkdir(parents=True, exist_ok=True)
    model_path = model_dir / f"{RUN_LABEL}.pth"
    ...
    result_row = {
        ...
        "use_focal_loss": True,
        "focal_gamma": FOCAL_GAMMA,
        "feature_set": "no_rolling_averages",
...
def main():
    print("=" * 80)
    print("HYBRID CNN — NO ROLLING AVERAGES ABLATION")
    print("=" * 80)
    ...
    result = run_experiment(device)
    df = pd.DataFrame([result])

    comparison_dir = PROJECT_ROOT / "artifacts" / "cnn_results"
    comparison_dir.mkdir(parents=True, exist_ok=True)
    out_path = comparison_dir / f"{RUN_LABEL}.csv"
