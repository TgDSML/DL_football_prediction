...
from src.models.gru_from_scratch import DualScratchGRUClassifier
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a dual-encoder scratch GRU baseline.")
    ...
    parser.add_argument("--early-stopping", type=parse_bool, default=False)
...
def report_path(experiment_name: str, report_dir: str | None = None) -> Path:
    ...
    return PROJECT_ROOT / "outputs/reports/gru" / f"{experiment_name}.txt"
...
def load_split(
    ...
    if not npz_path.exists() or not metadata_path.exists():
        raise FileNotFoundError(
            f"Missing {prefix} sequence files for {split_name}. Run `python scripts/build_sequences.py --sequence-length {prefix.removeprefix('home_away_seq')} --variants home_away` first."
        )
...
def validate_and_split_home_away(
    ...
    if [name.removeprefix("home_") for name in home_features] != [
        name.removeprefix("away_") for name in away_features
    ]:
...
def standardize_streams(
    ...
    home_std = train_home.std(axis=(0, 1), keepdims=True)
    home_std = np.where(home_std < 1e-6, 1.0, home_std)
    away_mean = train_away.mean(axis=(0, 1), keepdims=True)
    away_std = train_away.std(axis=(0, 1), keepdims=True)
    away_std = np.where(away_std < 1e-6, 1.0, away_std)
...
def train_model(
    ...
    early_stopping: bool,
    patience: int,
) -> tuple[DualScratchGRUClassifier, list[dict[str, float]], int, int]:
...
def build_report(
    ...
    lines = [
        ...
        f"Early stopping epoch: {stopped_epoch}",
    ...
    for epoch_metrics in history:
        lines.append(
            ...
            f"train_macro_f1={epoch_metrics['train_macro_f1']:.4f} "
...
def main() -> None:
    ...
    train_home, train_away = validate_and_split_home_away(train_X, feature_names)
    val_home, val_away = validate_and_split_home_away(val_X, feature_names)
    test_home, test_away = validate_and_split_home_away(test_X, feature_names)
    ...
    model, history, best_epoch, stopped_epoch = train_model(
        model=model,
